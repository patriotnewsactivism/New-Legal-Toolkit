...
import { Copy, Download, FileText, IdCard, Printer, RefreshCw, Palette } from "lucide-react";
...
const cx = (...cs: Array<string | false | undefined>) => cs.filter(Boolean).join(" "); // why: minimal runtime fn for conditional classes
...
type FormState = z.infer<typeof BaseFormSchema> & { timeLimit: string; statute: string; generated: string; styleMode: "classic" | "pro" };
...
const initialState: FormState = { ...BaseFormSchema.parse({}), documentType: "FOIA Request", agency: "", selectedState: "", jurisdiction: "", incident: "", recipient: "", damages: "", violationType: "harassment", claimType: "general", plaintiffName: "", defendantName: "", caseNumber: "", courtName: "", timeLimit: "", statute: "", generated: "", styleMode: "classic" };
...
export default function LegalToolkitPro(){
  ...
  const { isUltimate } = useSubscription();
  ...
  useEffect(() => { const { generated, ...rest } = state; localStorage.setItem("ltp-state", JSON.stringify(rest)); }, [state.documentType, state.selectedState, state.agency, state.violationType, state.claimType, state.styleMode]);
  ...
  const classic = state.styleMode === "classic";

  return (
    <div className={cx("mx-auto max-w-6xl space-y-6", classic ? "p-3" : "p-4")}> 
      {/* Header */}
      <div className={cx("flex flex-col gap-2 md:flex-row md:items-center md:justify-between",
        classic && "gap-1")}> 
        <div>
          <h1 className={cx("tracking-tight", classic ? "text-xl font-semibold" : "text-2xl font-semibold")}>Constitutional Rights — Legal Toolkit</h1>
          <p className={cx("text-muted-foreground", classic ? "text-xs" : "text-sm")}>Fast templates and references for records requests, speech, and accountability.</p>
        </div>
        <div className={cx("flex gap-2", classic && "gap-1")}> 
          <Select value={state.styleMode} onValueChange={(v)=>dispatch({type:"set", key:"styleMode", value:v})}>
            <SelectTrigger className={cx("w-[160px]", classic && "h-8 text-xs")}> 
              <SelectValue placeholder="Style" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="classic"><div className="flex items-center gap-2"><Palette className="h-4 w-4"/>Classic</div></SelectItem>
              <SelectItem value="pro"><div className="flex items-center gap-2"><Palette className="h-4 w-4"/>Pro</div></SelectItem>
            </SelectContent>
          </Select>
          <Button variant="secondary" size={classic ? "sm" : "default"} onClick={() => dispatch({ type: "reset" })}><RefreshCw className="mr-2 h-4 w-4" />Reset</Button>
          <Button size={classic ? "sm" : "default"} onClick={handleGenerate}><FileText className="mr-2 h-4 w-4" />Generate</Button>
        </div>
      </div>

      {/* Bundle & Pricing */}
      <Card className={cx(classic && "shadow-none border-muted/50 rounded-lg")}> 
        <CardHeader className={cx(classic && "py-3")}> 
          <CardTitle className={cx(classic ? "text-base" : "text-xl")}>Choose Your Plan</CardTitle>
          <p className={cx("text-muted-foreground", classic ? "text-xs" : "text-sm")}>Bundle with the Civil Rights Tool on the <span className="font-semibold">Ultimate</span> plan for team features and org branding.</p>
        </CardHeader>
        <CardContent className={cx(classic && "pt-0")}> 
          <PricingPlans />
        </CardContent>
      </Card>
      ...
      <Card className={cx(classic && "shadow-none border-muted/50 rounded-lg")}> 
        <CardHeader className={cx(classic && "py-3")}>
          <CardTitle className={cx(classic ? "text-base" : "text-xl")}>Request Builder</CardTitle>
        </CardHeader>
        <CardContent className={cx("space-y-4", classic && "space-y-3")}> 
          <div className={cx("grid gap-4 md:grid-cols-3", classic && "gap-3")}> 
            <div>
              <Label className={cx(classic && "text-xs")}>Document Type</Label>
              <Select value={state.documentType} onValueChange={(v)=>dispatch({type:"set", key:"documentType", value:v})}>
                <SelectTrigger className={cx("mt-1", classic && "h-8 text-xs")}> <SelectValue placeholder="Choose"/></SelectTrigger>
            ...
            <div>
              <Label className={cx(classic && "text-xs")}>State</Label>
              <Select value={state.selectedState as string} onValueChange={(v)=>dispatch({type:"set", key:"selectedState", value:v})}>
                <SelectTrigger className={cx("mt-1", classic && "h-8 text-xs")}><SelectValue placeholder="Select a state"/></SelectTrigger>
                <SelectContent className={cx("max-h-72", classic && "text-xs")}> 
                  {["", ...ALL_STATES].map((c)=> (<SelectItem key={c || "none"} value={c as string}>{c || "—"}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
            <div>
              <Label className={cx(classic && "text-xs")}>Agency / Recipient</Label>
              <Input className={cx("mt-1", classic && "h-8 text-xs")} value={state.agency} onChange={(e)=>dispatch({type:"set", key:"agency", value:e.target.value})} placeholder="e.g., City Clerk / Records Custodian"/>
            </div>
          </div>

          <div className={cx("grid gap-4 md:grid-cols-3", classic && "gap-3")}> 
            <div>
              <Label className={cx(classic && "text-xs")}>Jurisdiction</Label>
              <Input className={cx("mt-1", classic && "h-8 text-xs")} value={state.jurisdiction} onChange={(e)=>dispatch({type:"set", key:"jurisdiction", value:e.target.value})} placeholder="Auto‑filled by state"/>
            </div>
            <div>
              <Label className={cx(classic && "text-xs")}>Statute</Label>
              <Input className={cx("mt-1", classic && "h-8 text-xs")} value={state.statute} onChange={(e)=>dispatch({type:"set", key:"statute", value:e.target.value})} placeholder="Auto‑filled by doc type"/>
            </div>
            <div>
              <Label className={cx(classic && "text-xs")}>Time Limit</Label>
              <Input className={cx("mt-1", classic && "h-8 text-xs")} value={state.timeLimit} onChange={(e)=>dispatch({type:"set", key:"timeLimit", value:e.target.value})} placeholder="Auto‑filled by doc type"/>
            </div>
          </div>

          <div className={cx("grid gap-4 md:grid-cols-2", classic && "gap-3")}> 
            <div>
              <Label className={cx(classic && "text-xs")}>Subject / Incident</Label>
              <Textarea className={cx("mt-1", classic && "text-xs")} rows={classic ? 4 : 5} value={state.incident} onChange={(e)=>dispatch({type:"set", key:"incident", value:e.target.value})} placeholder="Describe records sought or facts (who/what/when/where)"/>
            </div>
            <div className="space-y-4">
              ...
              <div>
                <Label className={cx(classic && "text-xs")}>Damages (optional)</Label>
                <Textarea className={cx("mt-1", classic && "text-xs")} rows={classic ? 2 : 3} value={state.damages} onChange={(e)=>dispatch({type:"set", key:"damages", value:e.target.value})} placeholder="Monetary damages, reputational harm, etc."/>
              </div>
            </div>
          </div>

          <Tabs defaultValue="parties">
            <TabsList className={cx(classic && "h-8 text-xs")}> 
              <TabsTrigger value="parties">Parties</TabsTrigger>
              <TabsTrigger value="case">Case</TabsTrigger>
            </TabsList>
            <TabsContent value="parties" className={cx("grid gap-4 md:grid-cols-3", classic && "gap-3")}> 
              <div>
                <Label className={cx(classic && "text-xs")}>Plaintiff</Label>
                <Input className={cx("mt-1", classic && "h-8 text-xs")} value={state.plaintiffName} onChange={(e)=>dispatch({type:"set", key:"plaintiffName", value:e.target.value})}/>
              </div>
              <div>
                <Label className={cx(classic && "text-xs")}>Defendant</Label>
                <Input className={cx("mt-1", classic && "h-8 text-xs")} value={state.defendantName} onChange={(e)=>dispatch({type:"set", key:"defendantName", value:e.target.value})}/>
              </div>
              <div>
                <Label className={cx(classic && "text-xs")}>Recipient (for C&D/Subpoena)</Label>
                <Input className={cx("mt-1", classic && "h-8 text-xs")} value={state.recipient} onChange={(e)=>dispatch({type:"set", key:"recipient", value:e.target.value})}/>
              </div>
            </TabsContent>
            <TabsContent value="case" className={cx("grid gap-4 md:grid-cols-3", classic && "gap-3")}> 
              <div>
                <Label className={cx(classic && "text-xs")}>Claim Type</Label>
              ...
              <div>
                <Label className={cx(classic && "text-xs")}>Case Number</Label>
                <Input className={cx("mt-1", classic && "h-8 text-xs")} value={state.caseNumber} onChange={(e)=>dispatch({type:"set", key:"caseNumber", value:e.target.value})}/>
              </div>
              <div>
                <Label className={cx(classic && "text-xs")}>Court Name</Label>
                <Input className={cx("mt-1", classic && "h-8 text-xs")} value={state.courtName} onChange={(e)=>dispatch({type:"set", key:"courtName", value:e.target.value})}/>
              </div>
            </TabsContent>
          </Tabs>
        </CardContent>
      ...
      {/* Law chips */}
      <div className={cx("flex flex-wrap gap-2", classic && "gap-1 text-[11px]")}> 
      ...
      <div className={cx("grid gap-4 md:grid-cols-2", classic && "gap-3")}> 
        <Card className={cx(classic && "shadow-none border-muted/50 rounded-lg")}>
          <CardHeader className={cx(classic && "py-3")}><CardTitle className={cx(classic ? "text-base" : "text-xl")}>Preview</CardTitle></CardHeader>
          <CardContent>
            {state.documentType === "ID Rights Card" ? (
              ...
              <Textarea className={cx("min-h-[300px]", classic && "text-sm")} value={state.generated} onChange={(e)=>dispatch({type:"generate", value:e.target.value})}/>
            )}
          </CardContent>
        </Card>
        ...
        <Card className={cx(classic && "shadow-none border-muted/50 rounded-lg")}> 
          ...
          <CardContent className={cx("space-y-3")}> 
            ...
            <div className={cx("text-muted-foreground", classic ? "text-xs" : "text-sm")}>Tip: For deadlines like “5 business days,” a reminder from today ({fmtDate()}) might be <code>{fmtDate(addBusinessDays(new Date(), 5))}</code>.</div>
            {!isUltimate && (
              <div className={cx("rounded-md border p-3 text-sm", classic && "text-xs p-2 border-muted/50")}> 
            ...
            <div className={cx("rounded-md border p-3 text-xs leading-relaxed text-muted-foreground", classic && "p-2 border-muted/50")}> 
